...
心率数据会自动保存到 `heart_rate_data/` 目录，按日期创建 CSV 文件（如 `heart_rate_2025-12-06.csv`），方便后续离线分析。

### 数据导出

可通过编程方式调用 `HeartRateStats` 类的 `export_data()` 方法导出数据为 CSV、JSON 或 TXT 格式，支持按时间范围过滤。
